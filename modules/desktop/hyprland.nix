{
  ...
  flake.modules.hjem.linux =
    ...
    let
        ...
        on = [
          {
            _args = [
              "hyprland.start"
              # quickshell already runs as a user service bound to the
              # graphical session, so only the wallpaper daemon needs starting
              # here.
      ...
      # Hyprland's lua config is a sequence of `hl.<setting>(args)` calls, so the
      # attrset above is rendered rather than serialised: a list value repeats the
      # call, an `_args` entry spreads into multiple arguments instead of one
      # table, and a `_var` entry becomes a plain `local` hoisted above the calls.
      # These prefixes have to be emitted first because later calls reference what
      # they define.
      importantPrefixes = [
        "$"
        "bezier"
      ...
      localNames = lib.filter (name: settings.${name} ? _var) names;
      callNames = lib.filter (name: !(lib.elem name localNames)) names;

      importantNames = lib.unique (
        lib.concatMap (prefix: lib.filter (lib.hasPrefix prefix) callNames) importantPrefixes
      );
      orderedNames = importantNames ++ lib.filter (name: !(lib.elem name importantNames)) callNames;

      renderLocal =
        name: "local ${settings.${name}.name or name} = ${renderArgs settings.${name}._var}\n";
      ...
      renderCalls =
        ...
        let
          value = settings.${name};
        in
        lib.concatMapStrings (v: "hl.${name}(${renderArgs v})\n") (
          if lib.isList value then value else [ value ]
        );

      # Imported into systemd and D-Bus before hyprland-session.target is reached,
      # so user services inherit a usable session environment.
      sessionVariables = [
        "DISPLAY"
    ...
    in
    {
      ...
      xdg.config.files = {
        ...
        ''
        + section "settings.locals" (lib.concatMapStrings renderLocal localNames)
        + lib.concatMapStrings (name: section "settings.${name}" (renderCalls name)) orderedNames
        + section "startup" ''
          hl.on("hyprland.start", function()
            hl.exec_cmd(${toLua "${lib.getExe' pkgs.dbus "dbus-update-activation-environment"} --systemd ${lib.concatStringsSep " " sessionVariables} && systemctl --user stop hyprland-session.target && systemctl --user start hyprland-session.target"})
          end)
        ''
        + section "shutdown" ''
        ...
        # Points lua-language-server at the `hl` API that hyprland ships stubs for.
